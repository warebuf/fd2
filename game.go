		...
		if err := m.socket.ReadJSON(&msg); err == nil {
			...
			if msg.Event == "createMatch" {
				...
				if m.m.started == true && m.m.uuid_to_team_int[m.u.uid].a == msg.Team_index && m.m.uuid_to_team_int[m.u.uid].b == msg.Client_index {
					...
					if m.m.team_client_hero[msg.Team_index][msg.Client_index][msg.Hero_index].Health > 0 && m.m.team_client_hero[msg.Team_index][msg.Client_index][msg.Hero_index].Position == 0 {
							...
							for i := 0; i < len(m.m.team_client_hero); i++ {
								for j := 0; j < len(m.m.team_client_hero[i]); j++ {
									for k := 0; k < len(m.m.team_client_hero[i][j]); k++ {
										if m.m.team_client_hero[i][j][k].Bot == false && m.m.team_client_hero[i][j][k].Move < 0 {
											sim_check = false
										}
									}
								}
