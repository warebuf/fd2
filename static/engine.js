...
function drawStats() {
    ...
    for(let i = 0; i < match_data[current_state].length; i++) {
        for(let j = 0; j < match_data[current_state][i].length; j++) {
            for(let k = 0; k < match_data[current_state][i][j].length; k++) {
                ctx.fillText(i + " " +j + " " + k + " " + ((match_data[current_state][i][j][k].Health > 0) ? "active" : "rip") + " " + match_data[current_state][i][j][k].Direction + " " + match_data[current_state][i][j][k].Position + " -", 70, ((i+j)*115) + 55 + (k*20));
